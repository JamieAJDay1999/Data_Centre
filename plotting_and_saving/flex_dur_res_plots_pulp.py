...
def extract_detailed_results(model_vars, params, data, start_timestep, flex_time, baseline_df):
    """
    MODIFIED: This pulp-compatible function is now more robust, checking for
    the existence of columns in the baseline DataFrame before access.
    """
    time_slots = range(start_timestep, start_timestep + flex_time + 12)
    results_df = pd.DataFrame(index=list(time_slots))
    ...
    results_df['P_Total_kw_Opt'] = [
        model_vars['p_grid_it_kw'][s].value() +
        (model_vars['p_chiller_hvac_w'][s].value() / 1000.0) +
        (model_vars['p_chiller_tes_w'][s].value() / 1000.0) +
        model_vars['p_grid_od_kw'][s].value() +
        model_vars['p_ups_ch_kw'][s].value()
        for s in time_slots
    ]
    ...
    power_source_map = {
        'P_IT_Total_kW': model_vars['p_it_total_kw'], 'P_Grid_IT_kW': model_vars['p_grid_it_kw'],
        'P_Chiller_HVAC_kW': model_vars['p_chiller_hvac_w'], 'P_Chiller_TES_kW': model_vars['p_chiller_tes_w'],
        'P_Grid_Other_kW': model_vars['p_grid_od_kw'], 'P_UPS_Charge_kW': model_vars['p_ups_ch_kw'],
        'P_UPS_Discharge_kW': model_vars['p_ups_disch_kw'],
    }

    for source_name, var_dict in power_source_map.items():
        if 'Chiller' in source_name:
            opt_vals = [var_dict[s].value() / 1000.0 for s in time_slots]
        else:
            opt_vals = [var_dict[s].value() for s in time_slots]

        # ROBUSTNESS FIX: Check if column exists in baseline before trying to access
        base_vals = [baseline_df.loc[s, source_name] if s in baseline_df.index and source_name in baseline_df.columns else 0 for s in time_slots]
    ...
    hvac_opt_kw = [model_vars['p_chiller_hvac_w'][s].value() / 1000.0 for s in time_slots]
    tes_opt_kw = [model_vars['p_chiller_tes_w'][s].value() / 1000.0 for s in time_slots]
    opt_vals_cool_kw = [h + t for h, t in zip(hvac_opt_kw, tes_opt_kw)]
    base_vals_cool_kw = [baseline_df.loc[s, 'P_Grid_Cooling_kW'] if s in baseline_df.index and 'P_Grid_Cooling_kW' in baseline_df.columns else 0 for s in time_slots]
    ...
    # ... Other metrics (Energy, Temp, CPU) remain the same ...
    e_tes_opt_kwh = [model_vars['e_tes_kwh'][s].value() for s in time_slots]
    ...
    temp_source_map = {
        'T_IT_Celsius': model_vars['t_it'], 'T_Rack_Celsius': model_vars['t_rack'],
        'T_ColdAisle_Celsius': model_vars['t_cold_aisle'], 'T_HotAisle_Celsius': model_vars['t_hot_aisle']
    }
    
    for base_name, var_dict in temp_source_map.items():
        opt_vals = [var_dict[s].value() for s in time_slots]
    ...
    total_cpu_load_opt = [model_vars['total_cpu'][s].value() for s in time_slots]
    total_cpu_load_base = [baseline_df.loc[s, 'Total_CPU_Load'] if s in baseline_df.index and 'Total_CPU_Load' in baseline_df.columns else 0 for s in time_slots]
    
    flexible_cpu_usage = {s: 0 for s in time_slots}
    for (t_job, k_job, s_job), var in model_vars['ut_ks'].items():
        val = var.value()
...
# The plotting functions below do not need any changes.
def plot_flex_contribution_grid(all_plot_data, timesteps, flex_magnitudes):
    ...
    for plot_data in all_plot_data:
        ...
        pos_bottom, neg_bottom = np.zeros(len(diff_df)), np.zeros(len(diff_df))
        for name in series_order:
            if name not in diff_df.columns: continue
            s = diff_df[name].values
            color, s_pos, s_neg = palette.get(name), np.clip(s, 0, None), np.clip(s, None, 0)
        ...
        if global_max_dur > 0:
            longest_x, duration_labels_hours = np.arange(global_max_dur), np.arange(1, global_max_dur + 1) / 4.0
    ...
    extra_items = [Line2D([0], [0], ls='--', c='k', alpha=0.5, lw=2, label='Flex target'), Line2D([0], [0], marker='x', ls='None', c='#666', ms=8, label='Sum of diffs')]
    fig.legend(handles=legend_swatches + extra_items, loc='upper left', bbox_to_anchor=(0.87, 0.95), borderaxespad=0, title='Legend', fontsize=10, title_fontsize=14)
