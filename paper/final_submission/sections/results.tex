...
\caption{Reference and coordinated operation on 22 September 2025. ``Benchmark import'' denotes reference operation; the optimised trajectory is the undisturbed reference for the subsequent event assessment. Opening states are inherited from the preceding day.}
\label{fig:day}
\end{figure*}
...
Figure~\ref{fig:envelope} reports certified feasible duration for each requested deviation and start time. A 100 kW reduced-import request is sustained for six hours from noon, while a 150 kW reduction is sustained for five hours from 13:00. These requests correspond to 10\% and 15\% of rated IT power, while the actual target is defined at the whole-facility meter. A 25 kW increased-import request reaches the twelve-hour assessment cap at 03:00, 04:00, 08:00, 10:00, 11:00 and 12:00. These cap-reaching results establish twelve-hour feasibility under the recovery conditions; they do not establish an intrinsic upper limit of twelve hours.
...
Two assessment boundaries are important when reading the figure. First, an event cannot extend beyond the selected local day, although recovery may continue for three hours afterwards. The declining late-day durations consequently reflect this assessment limit as well as physical capability. Second, eight cells retain a plus sign because the next candidate duration remained unresolved. For example, the 25 kW increase at 06:00 is feasible for at least 10.25 h, and the 50 kW increase at 07:00 for at least 7.25 h. A solver timeout cannot establish that a longer response is impossible.
...
\caption{Component changes for selected reduced-import events. The dashed line identifies the requested deviation and ends at the certified duration. Grey space aligns panels of different lengths; recovery dispatch is not plotted. Recovery is verified by terminal constraints.}
\label{fig:reduction}
\end{figure*}
\begin{figure*}[t]
\centering\includegraphics[width=\textwidth]{Figure_8.png}
\caption{Component changes for selected increased-import events. Opposing component actions can be larger than the net requested deviation. A plus sign denotes an unresolved adjacent-duration trial. Grey space follows the certified event and contains no plotted recovery dispatch; the target is released during recovery.}
\label{fig:increase}
\end{figure*}

The reduced-import cases provide a particularly clear comparison. At 06:00, the selected 100 and 200 kW reductions last 1 h and are supplied predominantly by additional UPS discharge. Mean UPS contributions are approximately 99 and 199 kW, with small reductions in IT and direct cooling demand making up the response within the target tolerance. At 15:00, both requested reductions are sustained for 3 h, but deferred IT execution and reduced direct cooling supply most of the response. Their mean contributions are approximately 56 and 38 kW for the 100 kW request, and 121 and 74 kW for the 200 kW request. UPS discharge contributes approximately 6 and 5 kW on average, respectively. TES is full and idle in the undisturbed trajectory at that start, and neither case changes TES charging electricity during the event. Installed energy is therefore not a reliable guide to which resource will participate in an event.
...
Over the complete 2025 trajectory, reference operation costs GBP~550,503.59 and coordinated operation GBP~520,876.68. The resulting saving is GBP~29,626.91, or 5.382\%. These totals compare the same arriving workload and thermal operating limits; reference cooling is already cost optimised. The result therefore measures the additional electricity-cost benefit of scheduling eligible work and coordinating storage under the observed reference prices.

Annual grid electricity increases from 6.718 to 6.789 GWh, approximately 1.05\%, while peak import rises from 974.9 to 1,682.5 kW. Lower expenditure is consequently not evidence of lower energy consumption or a smoother load profile. The objective rewards the timing of electricity use. Conversion losses and changes in utilisation can increase energy use while the shift towards lower-priced intervals reduces cost. Peak charging and workload execution are also admissible when their combined import remains below the modelled structural limit. A real connection agreement or capacity charge could change the attractiveness of that schedule.

\input{tables/annual_components.tex}

Table~\ref{tab:cost-components} separates the signed electricity-cost terms. IT demand and direct CRAC cooling contribute reductions of approximately GBP~16,722 and GBP~14,425, respectively. TES charging costs GBP~6,897, and net UPS exchange reduces cost by GBP~5,377. Auxiliary expenditure is unchanged. These terms reconcile to the whole-facility saving but are not standalone asset profits. The positive TES charging term records the electricity used to produce stored cooling; its benefits appear partly in reduced direct chiller demand and in the opportunities it creates for the rest of the schedule. Similarly, the change in CRAC cost reflects the timing of IT heat as well as cooling decisions.
...
The annual sensitivities show increasing savings as flexible workload share, UPS energy capacity or TES energy capacity is increased over the tested ranges (Figure~\ref{fig:sensitivity}; full values in \supp{6}). Workload share has the largest response: halving it reduces savings to 3.737\%, while increasing it by half raises them to 6.633\%. UPS capacity spans 4.897--5.829\%, and TES capacity 5.129--5.499\%. This ordering applies to the selected parameter changes. It is not a comparison at equal investment cost, and it does not establish how simultaneous changes would interact.
\begin{figure*}[t]
...
UPS enlargement poses a more direct design question: how much additional expenditure could its additional operational saving support? Increasing capacity from 600 to 750 kWh reduces annual coordinated cost by GBP~1,274.75. Increasing it to 900 kWh reduces cost by GBP~2,459.89. These increments correspond to GBP~8.50 and GBP~8.20 per added nameplate kWh per year. The second 150 kWh increment is worth less than the first, although the taper is modest. Because the reserve fraction remains 50\%, these additions increase the operational energy window by only 75 and 150 kWh, respectively.

\input{tables/ups_screen.tex}

Under the illustrative ten-year, 8\% real-discount assumptions, Table~\ref{tab:ups-screen} gives supported incremental investments of approximately GBP~8,554 and GBP~16,506, equivalent to GBP~57.02 and GBP~55.02 per added nameplate kWh. These are break-even expenditure thresholds, not estimates of installed battery prices. They identify the amount available to cover enlargement if the observed annual savings persisted. Additional installation and operating costs must fit within that amount for the screen to be favourable.
