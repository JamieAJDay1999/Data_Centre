...
The daily workload profile is a modelling assumption informed by published workload and flexibility studies~\cite{Cao2022ApEn,6779441,Misaghian2023TIA,Liu2012PER}. It is not a measured trace of a particular facility. The adopted quarter-hour profiles specify flexible and inflexible utilisation and the allocation of flexible work among the four deferral classes. They repeat by local clock time throughout the year. \supp{2} provides all 96 input rows and explains the source interpretation, allowing reproduction without inferring values from a figure. Workload composition is an assumption tested by sensitivity, not a general estimate of the fraction of all data-centre work that is shiftable.
