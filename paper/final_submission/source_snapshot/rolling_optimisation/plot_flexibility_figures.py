...
def plot_components(
    ...
    fig.set_layout_engine('constrained', w_pad=0.07, h_pad=0.05)
    ...
    for row, magnitude in enumerate(magnitudes):
        for column, start in enumerate(starts):
            ...
            if (row, column) not in panels:
                axis.text(
                    0.5, 0.5, "No certified event", ha="center", va="center",
            ...
            axis.hlines(
                magnitude, 0, hours, color=VERMILION, linestyle=(0, (3.2, 1.4)),
                linewidth=1.0, zorder=4,
                label="Requested $\\Delta P$",
            )
            lower = bool(detailed[(start, magnitude)].attrs.get('duration_is_lower_bound', False))
            axis.text(.98, .96, f"Certified: {hours:g}{'+' if lower else ''} h",
...
def _load_detailed(
    ...
    for start in starts:
        for magnitude in magnitudes:
            ...
            planned.attrs["duration_is_lower_bound"] = bool(match.iloc[0]["duration_is_lower_bound"])
