...
def save(fig, path, **kwargs) -> None:
    ...
    # Render separately before replacing the published asset. This preserves
    # the previous figure if rendering fails and avoids truncating a file that
    # a Windows PDF/image preview may still be reading.
    path = Path(path)
    fd, temporary = tempfile.mkstemp(prefix=path.stem + '_', suffix=path.suffix, dir=path.parent)
    os.close(fd)
    try:
