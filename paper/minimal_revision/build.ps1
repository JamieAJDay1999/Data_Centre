...
try {
    ...
    foreach ($document in @('main','supplement')) {
        ...
        if ($LASTEXITCODE -ne 0) { Get-Content "build/$document-bibtex.txt" -Tail 25; throw "$document bibliography failed" }
        foreach ($pass in @(2,3)) {
            ...
            if ($LASTEXITCODE -ne 0) { Get-Content "build/$document-pass$pass.txt" -Tail 25; throw "$document pass $pass failed" }
        }
    ...
    Copy-Item -LiteralPath build/main.pdf -Destination (Join-Path $outputDirectory 'data_centre_minimal_revision.pdf') -Force
    Copy-Item -LiteralPath build/supplement.pdf -Destination (Join-Path $outputDirectory 'data_centre_minimal_supplement.pdf') -Force
}
