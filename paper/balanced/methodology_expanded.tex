...
and utilised as modelling inputs, capturing the time-dependent and duration-sensitive characteristics of DC flexibility in this study.
...
The model conceptualises all servers in the DC as one aggregate
...
Here $Q_t^{\mathrm{cool}}$ is the gross thermal cooling supplied by the direct
chiller and TES paths. The calibrated effective-airflow factor $\kappa$ accounts
in aggregate for mixing, bypass and nonuniform rack-flow effects in the
reduced-order thermal model. The cooling term acting on the four represented
thermal nodes is therefore defined as
$Q_t^{\mathrm{cool,eff}}\equiv\kappa Q_t^{\mathrm{cool}}$. This effective term
is a model calibration; it does not assert that the remaining fraction is an
independently measured cooling loss.
...
Q_t^{\mathrm{ITm}}
=Q_t^{\mathrm{out}}+Q_t^{\mathrm{IT}}-Q_t^{\mathrm{cool,eff}}
...
In this block, each $Q$ is expressed in kW,
$Q_t^{\mathrm{IT}}=P_t^{\mathrm{IT}}$, and
$Q_t^{\mathrm{cool,eff}}=\kappa Q_t^{\mathrm{cool}}$.
...
\emph{et al.}~\cite{cupelli2018data}, with its parameters scaled to 1 MW of IT
capacity. It describes the IT equipment, rack air,
cold aisle and hot aisle, while the chiller and TES determine the inlet air
temperature. The equations use consistent kW and kJ units. The heat capacities,
conductances, air mass flow, specific heat and effective-airflow factor used
below are listed in Table~\ref{tab:central-parameters}. The inlet
...
aisle. The factor $\kappa$ is a calibrated effective-airflow factor applied to
the nominal flow participating in the reduced-order node exchange. It captures
aggregate mixing, bypass and nonuniform rack-flow effects; it is not interpreted
as a direct measurement of a separate cooling-loss stream.
...
is the signed Intermittent Market Reference Price (IMRP) in GBP/MWh
\cite{lccc_imrp_actuals}. Calendar year reporting includes only committed
...
The model represents an aggregate DC with 1 MW of rated IT power. The
...
energy or computing capacity that the DC does not actually possess.
