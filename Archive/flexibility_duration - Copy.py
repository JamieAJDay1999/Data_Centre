...
def add_it_and_job_constraints(m, params, data, total_cpu, p_it_total, ut_ks):
    ...
    for s in params.TEXT_SLOTS:
        ...
        if s in params.T_SLOTS:
            ...
            m+=total_cpu[s]==data['inflexibleLoadProfile_TEXT'][s] + data['flexibleLoadProfile_TEXT'][s] + flexible_usage,f"TotalCPUUsage_EXT{s}"
...
def add_power_change_constraints(m, params, flex_target_kw, start_timestep, p_total, p_grid_it, p_chiller_hvac, p_chiller_tes, p_grid_od, p_ups_ch, flex_time, baseline_df):
    for t in range(start_timestep, start_timestep + flex_time):
        ...
        m += (p_grid_it[t] + 
            ...
            p_grid_od[t] + 
            p_ups_ch[t]) <= target_power, f"Power_Flex{t}"
...
def extract_contribs(m, baseline_df, start_timestep, flex_time, is_flex_down):
    contribs = []
    sources = ['Grid_IT', 'Chiller_HVAC', 'Chiller_TES', 'Grid_Other', 'UPS_Charge', 'UPS_Discharge', 'P_IT_Total']
    
    # Include the extended time period (flex_time + 12 steps buffer)
    extended_time = flex_time + 12
    
    for t in range(start_timestep, start_timestep + extended_time):
        row = {'timestep': t}
        total_opt = 0
        for source in sources:
            if source == 'Grid_IT':
                opt = pulp.value(m.variablesDict()[f'P_Grid_IT_{t}'])
                base = baseline_df.loc[t, 'P_Grid_IT_kW']
            elif source == 'Chiller_HVAC':
                opt = pulp.value(m.variablesDict()[f'P_Chiller_HVAC_Watts_{t}']) / 1000
                base = baseline_df.loc[t, 'P_Chiller_HVAC_kW']
            elif source == 'Chiller_TES':
                opt = pulp.value(m.variablesDict()[f'P_Chiller_TES_Watts_{t}']) / 1000
                base = baseline_df.loc[t, 'P_Chiller_TES_kW']
            elif source == 'Grid_Other':
                opt = pulp.value(m.variablesDict()[f'P_Grid_Other_{t}'])
                base = baseline_df.loc[t, 'P_Grid_Other_kW']
            elif source == 'UPS_Charge':
                opt = pulp.value(m.variablesDict()[f'P_UPS_Charge_{t}'])
                base = baseline_df.loc[t, 'P_UPS_Charge_kW']
            elif source == 'P_IT_Total':
                opt = pulp.value(m.variablesDict()[f'P_IT_Total_{t}'])
                base = baseline_df.loc[t, 'P_IT_Total_kW']
            elif source == 'UPS_Discharge':
                opt = pulp.value(m.variablesDict()[f'P_UPS_Discharge_{t}'])
                base = baseline_df.loc[t, 'P_UPS_Discharge_kW']
            diff = base - opt
        ...
        row['P_Total_base_kW'] = baseline_df.loc[t, 'P_Total_kW']
        row['P_Total_opt_kW'] = total_opt
        row['P_Total_diff_kW'] = row['P_Total_base_kW'] - row['P_Total_opt_kW'] 
...
def plot_contrib(df, start_ts, flex_kw, dur_steps):
    ...
    fig, ax = plt.subplots(figsize=(15, 7))  # Increased figure size for extended time
    plot_df.plot(kind='bar', stacked=True, ax=ax)
    
    # Add vertical line to separate flexibility period from extended time
    flex_period_end = dur_steps
    ax.axvline(x=flex_period_end-0.5, color='red', linestyle='--', alpha=0.7, linewidth=2, label=f'Flexibility Period End ({dur_steps} steps)')
    
    ax.set_title(f"Flexibility Contributions: Start {start_ts}, Flex {flex_kw} kW, Duration {dur_steps} steps (Extended Time Shown)")
    ax.set_ylabel("Power Deviation (kW)")
    ax.set_xlabel("Timestep")
    
    # Add horizontal dotted black line at flexibility magnitude
    ax.axhline(y=flex_kw, color='black', linestyle=':', linewidth=2, label=f'Target Flexibility: {flex_kw} kW')
    
    # Add scatter plot with grey "X"s at the sum of contributions
    total_flex = plot_df.sum(axis=1)
    ax.scatter(range(len(plot_df)), total_flex, color='grey', marker='X', s=50, label='Total Flexibility')
    
    # Add text annotation to explain the periods
    ax.text(0.02, 0.98, f'Flexibility Period: Steps {start_ts}-{start_ts+dur_steps-1}\nExtended Time: Steps {start_ts+dur_steps}-{start_ts+len(plot_df)-1}', 
             transform=ax.transAxes, verticalalignment='top', 
             bbox=dict(boxstyle='round', facecolor='lightblue', alpha=0.8))
    
    ax.legend()
    plt.tight_layout()
    filename = f"flex_contrib_start{start_ts}_flex{str(flex_kw).replace('-', 'neg')}_extended.png"
...
def plot_workload_composition(flex_df, contrib_df, baseline_df, params, data, start_ts, fm, dur_steps):
    """
    Generates a stacked bar chart of the optimized IT workload.
    - Black bars represent the original, baseline inflexible load.
    - Coloured bars represent all flexible jobs, with colors indicating the forward shift (lag).
    """
    # Use the full extended time period from the contributions DataFrame
    flex_period_index = contrib_df.index

    # --- MODIFIED SECTION START ---

    # 1. Get the TRULY original inflexible load from the baseline results DataFrame.
    # The 'Inflexible_Load_CPU_Nom' column in the baseline results captures the
    # inflexible workload from the very first simulation, before any optimizations.
    # .reindex() ensures alignment if the flex_period_index is shorter than the baseline.
    original_inflexible_load = data['inflexibleLoadProfile_TEXT'][:len(flex_period_index)]
    # 2. Pivot ALL flexible work by its lag (forward shift).
    # We no longer separate out "forced_flex" as it will now be displayed as a
    # colored bar with its corresponding maximum lag.
    ...
    # 3. Initialize the plot DataFrame with ONLY the original inflexible load.
    plot_df = pd.DataFrame({
        'Inflexible': original_inflexible_load
    }, index=flex_period_index)

    # --- MODIFIED SECTION END ---

    # Join the flexible load data, which is now correctly pivoted by lag.
    ...
    # Define colors: one for inflexible, and a spectrum for the flexible lags.
    ...
    # Plot baseline total for the extended period
    baseline_total = baseline_df.loc[plot_df.index, 'Total_CPU_Load'].values
    ax.plot(range(len(baseline_total)), baseline_total, label='Baseline Total Workload', linestyle='--', color='gray')

    # Add vertical line to separate flexibility period from extended time
    flex_period_end = dur_steps
    ax.axvline(x=flex_period_end - 0.5, color='red', linestyle='--', alpha=0.7, label=f'Flexibility Period End ({dur_steps} steps)')

    ax.set_title(f'Optimized IT Workload Composition for Flex: Start {start_ts}, Flex {fm} kW, Duration {dur_steps} steps (Extended Time Shown)')
    ...
    # Adjust tick frequency for extended time period
    tick_frequency = max(1, len(plot_df) // 20)
    ax.set_xticks(range(0, len(plot_df), tick_frequency))
    ax.set_xticklabels(plot_df.index[::tick_frequency], rotation=45, ha="right")

    # Add text annotation to explain the extended time
    ax.text(0.02, 0.98, f'Flexibility Period: Steps {start_ts}-{start_ts + dur_steps - 1}\nExtended Time: Steps {start_ts + dur_steps}-{start_ts + len(plot_df) - 1}',
             transform=ax.transAxes, verticalalignment='top',
             bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.8))

    fig.tight_layout(rect=[0, 0, 0.85, 1])
    plot_filename = f"it_load_stacked_start{start_ts}_flex{str(fm).replace('-', 'neg')}_extended.png"
            ...
            for fm in flex_magnitudes:
                ...
                if max_dur_steps > 0:
                    ...
                    if pulp.LpStatus[model.status] == 'Optimal':
                        ...
                        # Get data for the extended time period
                        extended_period_index = contrib_df.index
                        inflexible_base = pd.Series([data['inflexibleLoadProfile_TEXT'][s] for s in extended_period_index], index=extended_period_index)
                        forced_flex = flex_df[flex_df['shiftability'] == 0].groupby('processing_slot')['cpu_load'].sum().reindex(extended_period_index, fill_value=0)
                        
#
                        contrib_df['Inflexible_Load_CPU_Base'] = baseline_df['Inflexible_Load_CPU_Opt']
                        contrib_df['Flexible_Load_CPU_Base'] = baseline_df['Flexible_Load_CPU_Opt']
                        contrib_df['Total_CPU_Load_Base'] = baseline_df['Total_CPU_Load']
#

                        # Add optimized data for the extended period
                        contrib_df['Inflexible_Load_CPU_New'] = inflexible_base
                        contrib_df['Flexible_Load_CPU_New'] = flex_df.groupby('processing_slot')['cpu_load'].sum().reindex(extended_period_index, fill_value=0)
                        ...
                        contrib_df['Period_Type'] = 'Flexibility'
                        contrib_df.loc[contrib_df.index >= ts + max_dur_steps, 'Period_Type'] = 'Extended_Time'
                        
                        # Add workload shift information
                        contrib_df['Workload_Shift_From_Flex'] = contrib_df['Flexible_Load_CPU_New'] - contrib_df['Flexible_Load_CPU_Base']
                        
                        # Add summary statistics for extended time
                        flex_period_data = contrib_df[contrib_df['Period_Type'] == 'Flexibility']
                        extended_period_data = contrib_df[contrib_df['Period_Type'] == 'Extended_Time']
                        
                        if len(extended_period_data) > 0:
                            total_shift_to_extended = extended_period_data['Workload_Shift_From_Flex'].sum()
                            print(f"  Total workload shifted to extended time: {total_shift_to_extended:.2f} CPU units")
                        
                        csv_filename = f"flex_contrib_data_start{ts}_flex{str(fm).replace('-', 'neg')}_extended.csv"
...
if __name__ == '__main__':
    flex_magnitudes = [-100]   # negative for upward (reduce power), positive for downward (increase power)
